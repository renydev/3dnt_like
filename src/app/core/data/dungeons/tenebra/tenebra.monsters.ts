...
// Losango compacto (1-2-3-4-3-2-1) — ver tenebra.config.ts e o relatório de
...
export const TENEBRA_ROOM_ENEMIES: Record<number, RoomEnemyGroup> = {
  1: (scale) => [spawnMonster('bodak', scale)],
  3: (scale) => Array.from({ length: Math.max(1, d(6) - 4) }, () => spawnMonster('aparicao', scale)),
  6: (scale) => [spawnMonster('troglodita_guerreiro', scale)],
  10: (scale) => [spawnMonster('vulto_alado', scale)],
  13: (scale) => [spawnMonster('esqueleto_colossal', scale, true)],
  15: (scale) => [
    spawnMonster('ravarimm', scale, true),
  ],
};
...
export function rollTenebraEncounter(scale: GrowthScale): Enemy[] {
  const pool = ['bodak', 'aparicao', 'troglodita_guerreiro', 'vulto_alado', 'esqueleto_colossal'];
