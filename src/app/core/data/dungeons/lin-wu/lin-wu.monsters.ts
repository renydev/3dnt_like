...
// Losango compacto (1-2-3-4-3-2-1) — ver lin-wu.config.ts e o relatório de
...
export const LIN_WU_ROOM_ENEMIES: Record<number, RoomEnemyGroup> = {
  1: (scale) => [spawnMonster('artista_marcial', scale)],
  3: (scale) => Array.from({ length: Math.max(1, d(6) - 4) }, () => spawnMonster('artista_marcial', scale)),
  6: (scale) => [spawnMonster('artista_marcial', scale)],
  7: (scale) => Array.from({ length: Math.max(1, d(6) - 4) }, () => spawnMonster('artista_marcial', scale)),
  10: (scale) => [spawnMonster('artista_marcial', scale)],
  13: (scale) => [spawnMonster('artista_marcial', scale, true)],
  15: (scale) => [
    spawnMonster('yon_ude', scale, true),
  ],
};
...
export function rollLinWuEncounter(scale: GrowthScale): Enemy[] {
  const pool = ['artista_marcial', 'artista_marcial', 'artista_marcial', 'artista_marcial', 'artista_marcial'];
