...
// Losango compacto (1-2-3-4-3-2-1) — ver tauron.config.ts e o relatório de
...
export const TAURON_ROOM_ENEMIES: Record<number, RoomEnemyGroup> = {
  1: (scale) => [spawnMonster('guerreiro_tauron', scale)],
  3: (scale) => Array.from({ length: Math.max(1, d(6) - 4) }, () => spawnMonster('guerreiro_tauron', scale)),
  6: (scale) => [spawnMonster('guerreiro_tauron', scale)],
  10: (scale) => [spawnMonster('leao_gigante', scale)],
  13: (scale) => [spawnMonster('quimera', scale, true)],
  15: (scale) => [
    spawnMonster('potentius', scale, true),
  ],
};
...
export function rollTauronEncounter(scale: GrowthScale): Enemy[] {
  const pool = ['guerreiro_tauron', 'guerreiro_tauron', 'guerreiro_tauron', 'leao_gigante', 'quimera'];
